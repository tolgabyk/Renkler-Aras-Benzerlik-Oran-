...
def arrange(item, dim,min,max):
    ...
    for i in range(dim): #boyut kadar döner 
        if(i <= item_count):
...
benzerlik_orani = vectorSimilarity(siyah_normalized,siyah_normalized) # benzerlik oranı aranılıcak renkler

print(f"Oran = %{benzerlik_orani*100}")